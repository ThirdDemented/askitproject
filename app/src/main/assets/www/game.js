...
const carPool=[
 {id:'buick',y:1998,n:'Buick LeSabre Custom',p:1450,mi:184221,rel:84,mpg:24,tank:18,cargo:9,s:'Dale',color:'#a8aa9c',issues:['slow coolant seep','rear window sticks'],ad:'Runs good. AC needs charged. 3800 motor. Elderly owned. CASH ONLY.'},
 {id:'lincoln',y:2005,n:'Lincoln Town Car Signature',p:1900,mi:212904,rel:78,mpg:20,tank:19,cargo:11,s:'Ron',color:'#c2c2c2',issues:['coolant staining near intake','air suspension compressor sounds tired'],ad:'Highway miles. Leather. Cold AC. Grandma car. No lowballs.'},
 {id:'altima',y:2007,n:'Nissan Altima 2.5 S',p:1650,mi:168440,rel:61,mpg:27,tank:20,cargo:7,s:'Trevor',color:'#9ba2a8',issues:['CVT shudders when hot','front tire shoulder wear'],ad:'FAST reliable daily. New battery. Just drove to Chicago.'},
 {id:'camry',y:1999,n:'Toyota Camry LE',p:2300,mi:224501,rel:91,mpg:27,tank:18,cargo:7,s:'Marlene',color:'#b39b7f',issues:['small valve-cover seep','driver handle requires technique'],ad:'One owner. Has dents. Full maintenance folder.'},
 {id:'bmw',y:2008,n:'BMW 328i',p:1800,mi:137902,rel:49,mpg:23,tank:16,cargo:6,s:'Cody',color:'#5c6873',issues:['electric water pump weak','coolant warning was cleared'],ad:'MUST SELL. Runs perfect. Only needs coolant sensor.'},
 {id:'crownvic',y:2004,n:'Ford Crown Victoria LX',p:1750,mi:196211,rel:82,mpg:19,tank:19,cargo:10,s:'Curtis',color:'#d0c7b1',issues:['power steering seep','blend door stuck warm'],ad:'Former county car NOT POLICE. V8. New tires. Will run forever.'},
 {id:'focus',y:2011,n:'Ford Focus SE',p:2200,mi:156880,rel:72,mpg:30,tank:12,cargo:6,s:'Brenda',color:'#566d84',issues:['motor mount vibration','rear bearing hum'],ad:'Good commuter. Great MPG. Son bought truck. Price firm-ish.'},
 {id:'impala',y:2003,n:'Chevrolet Impala LS',p:1300,mi:201331,rel:68,mpg:23,tank:17,cargo:8,s:'Mike',color:'#907b72',issues:['wheel hub sensor intermittent','fuel gauge optimistic'],ad:'Starts runs stops. Heat works. 200k mostly highway.'},
 {id:'accord',y:2006,n:'Honda Accord LX',p:2600,mi:233104,rel:88,mpg:29,tank:17,cargo:7,s:'Keisha',color:'#9c9c99',issues:['AC clutch noisy','front rotors warped'],ad:'High miles but Honda. New alternator. Clean title.'},
 {id:'pontiac',y:2002,n:'Pontiac Grand Prix GT',p:1200,mi:191455,rel:64,mpg:22,tank:17,cargo:7,s:'Shawn',color:'#7d2f32',issues:['cooling fans only work on high','rear struts exhausted'],ad:'Strong 3800. Small exhaust leak. NEED GONE.'}
];
const supplyDefs=[
 {id:'toolkit',n:'BASIC TOOLKIT',cost:48,d:'Improves roadside repair attempts.'},{id:'coolant',n:'COOLANT',cost:18,d:'Can save an overheating car.'},{id:'oil',n:'2 QT OIL',cost:17,d:'Buys time if the engine gets thirsty.'},{id:'food',n:'ROAD FOOD',cost:32,d:'Reduces hunger for several legs.'},{id:'charger',n:'PHONE CHARGER',cost:14,d:'Protects navigation during a phone event.'},{id:'atlas',n:'PAPER ROAD ATLAS',cost:12,d:'Helps when navigation fails.'},{id:'fixflat',n:'FIX-A-FLAT',cost:13,d:'One desperate tire repair.'},{id:'blanket',n:'BLANKET',cost:22,d:'Makes sleeping in the car less miserable.'}
];
const legalGoods=[
...
function sfx(k){if(!soundOn)return;if(k==='ui')tone(note.C5,.04);if(k==='cash'){tone(note.C5,.05);tone(note.E5,.07,'square',.06,.06)}if(k==='car'){tone(note.E2,.09,'sawtooth',.08);tone(note.G2,.11,'square',.04,.06)}if(k==='bad'){noise(.08,.12);tone(note.C2,.18,'square',.08)}if(k==='cop'){tone(note.A4,.1);tone(note.E5,.1,'square',.06,.1);tone(note.A4,.1,'square',.06,.2);tone(note.E5,.1,'square',.06,.3)}if(k==='win'){[note.C4,note.E4,note.G4,note.C5].forEach((f,i)=>tone(f,.12,'square',.06,i*.08))}}
function musicTick(){if(!soundOn||!audioCtx)return;const t=themes[currentTheme]||themes.title,beat=60/t.bpm/2,m=t.m[musicStep%t.m.length],b=t.b[Math.floor(musicStep/2)%t.b.length];if(m)tone(note[m],beat*.7,'square',.035,0,'music');if(musicStep%2===0&&b)tone(note[b],beat*1.2,'triangle',.03,0,'music');musicStep++}
function startMusic(theme=currentTheme){currentTheme=theme;if(!soundOn||!ensureAudio())return;clearInterval(musicTimer);musicStep=0;musicTick();musicTimer=setInterval(musicTick,(60/(themes[currentTheme]?.bpm||110)/2)*1000)}
...
/* setup */
function initSelects(){cities.forEach((c,i)=>{for(const id of ['originSelect','destSelect']){const o=document.createElement('option');o.value=i;o.textContent=c.n;$(id).appendChild(o)}});$('originSelect').value='0';$('destSelect').value='4'}
...
function startNewLife(){clearSave();$('originCustom').value='';$('destCustom').value='';state=blankState();$('ageRange').value=31;$('ageOut').textContent='31';$('originSelect').value='0';$('destSelect').value='4';renderSetupCards();show('setupScreen')}
...
async function beginLife(){if(!state.career||!state.reason){alert('Choose a career and a reason for moving.');return}const btn=$('beginLifeBtn');btn.disabled=true;const oldText=btn.textContent;btn.textContent='FINDING ROUTE…';try{state.age=Number($('ageRange').value);let oi=Number($('originSelect').value),di=Number($('destSelect').value);const [oc,dc]=await Promise.all([resolveCustomCity($('originCustom').value),resolveCustomCity($('destCustom').value)]);if($('originCustom').value.trim()&&!oc){alert('I could not find that starting city. Try “City, State”.');return}if($('destCustom').value.trim()&&!dc){alert('I could not find that destination. Try “City, State”.');return}if(oc){cities.push(oc);oi=cities.length-1}if(dc){cities.push(dc);di=cities.length-1}state.origin=oi;state.dest=di;if(cities[state.origin].n===cities[state.dest].n){alert('Choose a different destination.');return}state.totalMiles=routeMiles();state.cash=Math.max(250,state.career.cash+state.reason.bonus);ensureTripStats();state.stats.startingCash=state.cash;state.days=state.reason.days;state.limits={shifts:state.career.shifts,ot:state.career.ot,parents:1,plasma:2,scratch:2,loan:1,stipend:1};rollAssets();rollListings();state.log=[`You are ${state.age}, working as ${state.career.n.toLowerCase()}.`,`You need to get from ${cities[state.origin].n} to ${cities[state.dest].n}: about ${state.totalMiles.toLocaleString()} road miles.`];renderPrep();show('prepScreen');refineRouteMiles()}finally{btn.disabled=false;btn.textContent=oldText}}
...
const roadEvents=[
 ...
 {id:'fuel',family:'stop',tag:'FUEL',eligible:s=>s.fuel<42,title:'LOW FUEL.',body:()=> 'A gas station sign appears ahead. Fuel is expensive. Walking is worse.',choices:[['FILL TANK',s=>{if(s.cash<1){endGame('STRANDED','YOU CANNOT AFFORD FUEL.',`The tank is nearly empty and your cash is gone.\nDistance traveled: ${Math.round(s.distance)} miles.`);return'You cannot buy fuel.'}const c=Math.min(s.cash,Math.round((100-s.fuel)/100*72));s.cash-=c;statSpend(c,'fuelSpent');state.stats.fuelStops++;s.fuel=clamp(s.fuel+(c/72)*100);return`You spend ${money(c)} on fuel.`}],['ADD $20',s=>{if(s.cash<1){endGame('STRANDED','YOU CANNOT AFFORD FUEL.',`The tank is nearly empty and your cash is gone.\nDistance traveled: ${Math.round(s.distance)} miles.`);return'You cannot buy fuel.'}const c=Math.min(20,s.cash);s.cash-=c;statSpend(c,'fuelSpent');state.stats.fuelStops++;s.fuel=clamp(s.fuel+(c/72)*100);return`You add ${money(c)} worth of fuel.`}],['KEEP GOING',()=> 'You pass the station. The gauge notices.']]},
 {id:'tire',family:'mechanical',tag:'MECHANICAL',eligible:s=>s.distance>80,title:'THUMP. THUMP. THUMP.',body:()=> 'The steering wheel starts vibrating. Something is wrong with a tire.',choices:[['STOP NOW',s=>useItem('fixflat')?'Fix-a-Flat keeps you moving.':(s.cash=Math.max(0,s.cash-85),'The tire costs you $85.')],['KEEP GOING',s=>{state.stats.warningsIgnored++;s.condition=clamp(s.condition-10);return'The vibration becomes part of the soundtrack.'}]]},
...
roadEvents.push(
 {id:'windshield',family:'mechanical',tag:'ROAD DAMAGE',eligible:s=>s.distance>90,title:'CRACK.',body:()=> 'A stone snaps off a truck tire and puts a bright new star in the windshield.',choices:[['IGNORE IT',s=>{s.morale=clamp(s.morale-2);return'The crack becomes part of the scenery.'}],['REPAIR AT NEXT STOP — $45',s=>{s.cash=Math.max(0,s.cash-45);return'A resin patch keeps it from spreading.'}]]},
...
$('departBtn').onclick=depart;$('prepAgainBtn').onclick=()=>{renderPrep();show('prepScreen')};$('driveLegBtn').onclick=driveLeg;$('marketStopBtn').onclick=()=>{ensureTripStats();state.stats.stops++;renderTrade();show('tradeScreen')};$('leaveMarketBtn').onclick=()=>{renderRoadHud();show('roadScreen')};$('againBtn').onclick=startNewLife;
...
function setArt(id,key){const img=$(id);if(!img)return;const src='art/'+key+'.webp';if(img.getAttribute('src')!==src){img.src=src;img.alt=sceneAlt[key]||('Illustrated '+key.replaceAll('-',' ')+' scene');}img.closest('.scene')?.setAttribute('data-scene',key)}
const sceneAlt={drive:'The highway through a detailed sedan cockpit.',gas:'A rural fuel station at blue hour.',diner:'A hot meal in a roadside diner.',motel:'Neon lights at a roadside motor court.',mechanic:'Tools and an open service bay in a repair shop.',rest:'A quiet interstate picnic shelter.',breakdown:'Steam rises from an engine at the roadside.',arrival:'Moving boxes at a welcoming new home.',failure:'A warning triangle and suitcase on a dark highway.',trade:'Secondhand goods at a small-town market.',underground:'An anonymous barter meeting behind a pawn shop.'};
const eventScenes={fuel:'gas',food:'diner',motel:'motel',repair:'mechanic',restarea:'rest',vending:'rest',overlook:'rest',trucker:'diner',marketcontact:'gas',cargoOffer:'gas',cargoScam:'trade',hail:'gas',battery:'breakdown',heat:'breakdown',tire:'breakdown',alternator:'breakdown',exhaust:'breakdown',windshield:'breakdown'};
...
function updateRoadEffects(){const scene=$('roadArt')?.closest('.scene');if(!scene)return;scene.classList.toggle('weather-rain',state.scene==='drive'&&state.weather.includes('RAIN'));scene.classList.toggle('weather-fog',state.scene==='drive'&&state.weather==='FOG');scene.classList.toggle('night',state.scene==='drive'&&isNight());$('fuelNeedle').style.transform='rotate('+(-65+state.fuel*1.3)+'deg)'}
function placeRoadMeters(){const landscape=matchMedia('(orientation:landscape) and (min-width:700px)').matches;const meters=document.querySelector('.compact-road-panel');const column=document.querySelector(landscape?'.road-visual-column':'.road-control-column');if(landscape)column.append(meters);else column.insertBefore(meters,document.querySelector('.road-event'));}
window.addEventListener('resize',placeRoadMeters);
...
function choiceCost(e,label){const fixed={motel:{'GET A ROOM':78},food:{'DINER — $22':22,'USE ROAD FOOD':hasItem('food')?0:9},repair:{'FULL REPAIR':300,'CHEAP FIX':95},tire:{'STOP NOW':hasItem('fixflat')?0:85},alternator:{'FIND A PARTS STORE':190}};return fixed[e.id]?.[label]??Number(label.match(/\$(\d+)/)?.[1]||0)}
function resume(){
 try{
  const saved=JSON.parse(localStorage.getItem(SAVE_KEY));if(!saved)return;
  state={...blankState(),...saved};ensureTripStats();
  if(state.cityData){cities.push(state.cityData.origin,state.cityData.dest);state.origin=cities.length-2;state.dest=cities.length-1;}
...
function boot(){try{soundOn=localStorage.getItem('lwh-sound')!=='false'}catch(e){}$('soundBtn').textContent=soundOn?'♪ SOUND ON':'♪ SOUND OFF';initSelects();renderSetupCards();try{if(localStorage.getItem(SAVE_KEY))$('resumeBtn').hidden=false}catch(e){}}
boot();
})();
